...
All Gherkin steps that match a [supported DSL](/docs/dsl/supported-dsl) are considered DSL steps by Gwen and are readily executable.
...
Confine all DSL steps to meta. Although it's possible to use them directly in features, we do not recommend making them DSL-aware. That's what meta is for!

:::

Use DSL steps to compose StepDefs and make your feature steps executable.

For example, to make the following step executable:
...
- [I navigate to `url`](/docs/dsl/supported-dsl#i-navigate-to-url)
- [`the page title <should|should not> <be|contain|start with|end with|match regex|match xpath|match template|match template file> "<expression>"`](https://github.com/gwen-interpreter/gwen-web/wiki/Supported-DSL#the-page-title-shouldshould-not-becontainstart-withend-withmatch-regexmatch-xpathmatch-templatematch-template-file-expression)
