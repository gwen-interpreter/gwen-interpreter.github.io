...
```gherkin {1,4} showLineNumbers
    Given first name can be located by css ".first_name"
      And capitalised name is defined by js
          """
          (name = first name) => { 
...
We can then apply it to different names by binding each one. In the following example, line 3 binds the `name` argument to the value returned by `first name`, applies our `capitalise` function (above) to that and stores the resulting value in `capitalised first name`. Line 4 does the same but with `last name` instead.

```gherkin {3-4} showLineNumbers
  Given first name can be located by css ".first_name"
    And last name can be located by css ".last_name"
    And capitalised first name is defined by capitalise applied to "${first name}"
    And capitalised last name is defined by capitalise applied to "${last name}"
```
...
Suppose we wanted to reuse the above functions to captitalise both the first and last names and combine them into a capitalised full name?
...
```gherkin {2}
  And full name is defined by js "(name, surname) => name + ' ' + surname"
  And capitalised full name is defined by full name applied to "${capitalised first name},${capitalised last name}" delimited by ","
```
...
A more complex example follows. If `name` resolves to 'GWEN' and `surname` resolves to 'Butterfly', then the following function would set `fancy full nane` to 'BUFFERFLY, Gwen'.
