...
Error messages raised by a series [soft](#soft-assertions) or [sustained](#sustained-assertions) assertions can be aggregated into a single error with an [accumulated assert](/docs/dsl#accumulated-asserts).
...
### Usage
 ...
 <p>Example</p>
  ...
  You can also [reset accumulated](/docs/dsl#i-reset-accumulated-errors) errors if you need to start a new series of accumulated assertions after the current one has completed.
