...
- Introduced `empty` literal that can be used interchangeably with `blank` in all [DSL steps](/docs/dsl) where `blank` is accepted.
- Introduced [`I attach "<filepath>"`](/docs/dsl/actions/file-actions#i-attach-filepath) DSL for attaching files to Gwen reports (where the name of the attachment will be the name of the file)
- Introduced [`I log record to <resultsFileId> file`](/docs/dsl/actions/file-actions#i-log-record-to-file) DSL for explicitly logging records to [results files](/docs/reports/csv#results-files)
