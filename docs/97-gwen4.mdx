...
You can now configure one or many automated [processes](/docs/processes#process-settings), each with its own launch options and [settings](/docs/settings). This enables you to configure unique processes in your project so you don't have to externally manage how they are launched. You simply give each process a name and define its configuration in a same named settings file. Then when you're ready to launch, you specify the name of the process you want to run with the [`-p|--process`]((/docs/launcher#gwen-launch-options-process)) option.
...
All input data sourced from files is now read only and cannot be overriden.
...
- `gwen-web-accept-untrusted-certs` setting (use <code>acceptInsecureCerts</code> [capability](/docs/settings#gwen-web-capabilities) instead)
