...
<ul>
  ...
  <li>
    ...
    <ul>
      <li><code>Feature</code> - To log feature level results to the CSV file when a feature completes</li>
      <li><code>Scdenario</code> - To log scenario level to the CSV file when a scenario completes</li>
      <li>Or blank to not log any records automatically (used for manually managed CSV logging)</li>
    </ul>
  </li>
  <li>
...
<ul>
  ...
  <li>
    ...
    <ul>
      ...
      <li>If blank, will use the value refered to by variable of same name as <code>&lt;fieldName&gt;</code></li>
      <li>
        If <code>&lt;fieldName&gt;</code> is set to <code>*</code>, then:
        <ul>
          <li>This reference must specify a path to the CSV template file containing the names of all fields to bind in the first row (and the name of such fields will also be used as the binding references)</li>
          <li>Can also be set to the special reference <code>$&lt;gwen.options.dataFile&gt;</code> which will resolve to the input CSV (data feed) file passed to Gwen via the [<code>-i|--input-data &lt;file&gt;</code>](/docs/cli#gwen-cli-options-input-data) CLI option. In the case all fields provided in the input file will be included and resolved by name.</li>
        </ul>
      </li>
    </ul>
  </li>
...
<Tabs
 ...
 In this default configuration, fields are uniquely defined upfront and reused across file definitions.
  ...
  In this redendant configuration, fields are replicated across file definitions.
...
<TabItem value="json">
  ...
  In this redunant configuration, fields are replicated across file definitions (note: field reuse is only supported in <code>conf</code> format and not <code>json</code>).
...
In this redunant configuration, fields are replicated across file definitions (note: field reuse is only supported in <code>conf</code> format and not <code>properties</code>).
