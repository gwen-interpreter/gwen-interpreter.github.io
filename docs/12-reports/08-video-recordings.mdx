...
Video recordings of browser sessions can be captured when running [Gwen in Docker](/docs/docker), and if either the [`GWEN_VIDEO`](/docs/launcher#environment-variables) environment variable or the `se:recordVideo` capability setting are set to `true`. Under these conditions, MP4 video recordings of each session will be attached to [HTML reports](/docs/reports/html) when execution completes.
...
:::caution Limitation
Videos will not be recorded when [headless browser](/docs/settings#gwen-web-browser-headless), or interactive (non [--batch/-b](/docs/launcher#gwen-launch-options-batch)) modes are enabled.
:::
