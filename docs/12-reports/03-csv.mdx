...
<ul>
  ...
  <li>
    ...
    <ul>
      ...
      <li>Omit the scope to not log any results automatically; for when you want to explicitly control the scope and where the logging will occur using a [`@Results`](#results-annotation) annotation or the [`I log record to <resultsFileId> file`](/docs/dsl#i-log-record-to-file) DSL</li>
    </ul>
  </li>
  <li>
    <code>[status]</code> - (Optional, only required if above scope sttribute is set) is either:
    <ul>
...
```json
{ name = "<fieldName>", ref = "[reference]", defaultValue = "[value]", excludes = "[excludeFields]" }
```
