...
> See also: [Video capture](/docs/docker#video-capture) 

Captured video recordings of browser sessions are currently only supported when running Gwen in [Docker with Selenoid](/docs/docker), and if either the [`GWEN_VIDEO`](/docs/cli#environment-variables) environment variable or `gwen.web.capability.enableVideo` setting are set to `true`. Under these conditions, MP4 video recordings of each session will be attached to [HTML reports](/docs/reports/html) when execution completes.
