...
Captured video recordings of browser sessions are currently only supported when running Gwen in [Docker with Selenoid](/docs/docker), and if the `gwen.web.capability.videoEnabled` setting is set to `true`. Under these conditions, MP4 video recordings of each session will be attached to [HTML reports](/docs/reports/html) when execution completes.
