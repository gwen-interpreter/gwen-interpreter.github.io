...
Pretty execution results and statistics are logged to the console in real time in sequential mode and deferred in [parallel](/docs/modes/parallel).
...
The default logging depth is 1, meaning that only top level feature steps are logged. This can be overriden with the following setting to include meta steps too:
- [`gwen.report.console.log.depth`](/docs/settings/reference#gwen-report-console-log-depth)
  - Default value is `1`
  - Accepts a number representing the step depth to log to
