...
- [`<element> can be located by <selector> "<expression>"`](/docs/dsl/bindings/element-locators#element-can-be-located-by-selector-expression)
- [`<textRef> should[ not] <match> "<expression>"`](/docs/dsl/assertions/text-asserts#text-ref-should-match-expression)
...
In this example, the `module-2.meta` and `module-3.meta` files will be loaded (in that order) before `spec-3.feature` is executed.  Again, this would happen without you having to specify any meta in the CLI call to Gwen.
