...
Although it's possible to specify imports directly in feature files, we do not recommend you doing so since it would introduce automation concerns into your features. Confining imports to associative meta instead will keep your features clean and untouched. You must enable [associative meta](/docs/settings/reference#gwen-associative-meta) to use this approach (note that this setting is enabled by default in since <Link to="docs/gwen3">Gwen 3</Link>).
