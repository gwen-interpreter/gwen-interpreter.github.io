...
Utility functions use a similar syntax to [JS arrow functions](/docs/js-functions#arrow-syntax) but with a `:=>` arrow instead of `=>`.
...
If you define a selector for a parent element, you can locate the child relative to the parent by reusing it.
...
You can also exeucte utility functions directly in the [REPL console](/docs/repl).
