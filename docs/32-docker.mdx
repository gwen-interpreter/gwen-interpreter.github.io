...
> Since [v3.16.0](https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.16.0), the [`init --docker`](/docs/launcher#gwen-launch-init-options-docker) command was introduced to generate Docker and Selenoid files in your project. You will need to create the files manually if you are using an older version.
...
The init --docker commad will create a `gwen/conf/browsers/browsers.json` file with the following content that will be used by Selenoid to stand up the browsers in Docker.
...
The init --docker commad will create a `gwen/conf/browsers/selenoid.conf` file with the following content that will be used by Gwen to connect to Selenoid.
...
```shell
SELENIUM_HUB=localhost yarn gwen -b -c gwen/conf/browsers/selenoid.conf gwen/features
```
...
File: *gwen/conf/browsers/selenoid.conf*
...
:::caution Limitation
Videos will not be recorded when [parallel execution](/docs/parallel), [headless browser](/docs/settings#gwen-web-browser-headless), or interactive (non [--batch/-b](/docs/launcher#gwen-launch-options-batch)) modes are enabled.
:::


## Downloading Files in Chrome

Configure the following in your `conf/browsers/selenoid.conf` file to enable automatic file downloads for Chrome in the Selenoid/Docker container.
