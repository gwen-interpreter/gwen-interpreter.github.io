...
This will resolve to the same outline at runtime:
...
The data can also be filtered using a `where` clause. For example, to get all the records where the `Status` column matches `pending`:
...
String interpolation syntax can also be used to reference a value in the current scope. For example, to get all the records where the `Status` column matches the value bound to an in-memory attribute named `SelectedStatus`:
