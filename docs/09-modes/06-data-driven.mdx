...
JSV data files can be passed into the interpreter to perform feature execution over multiple data sets. The values in each record will be bound to attributes in the feature scope by their names. Feature steps can reference the bound data using these names.
...
```json
[
  { "FIRST_NAME": "Gwen",    "LAST_NAME": "Stefani", "JOB": "Pop Singer"      },
  { "FIRST_NAME": "Gwyneth", "LAST_NAME": "Paltrow", "JOB": "Actress"         },
  { "FIRST_NAME": "Gwen",    "LAST_NAME": "Cooper",  "JOB": "Torchwood Agent" }
]
```
...
Note: Interpolation of JSON fields `${FIRST_NAME} ${LAST_NAME}` is supported in the feature name and description too.

:::

> The current JSON record number (starting at 1) is available as an implicit attribute named `data record number` and can be accessed by reference or [interpolation](/docs/advanced/string-interpolation).
