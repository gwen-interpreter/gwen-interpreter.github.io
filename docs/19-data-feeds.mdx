...
> Since [v3.64.0](https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.64.0)
...
```gherkin
  And job 1 is defined by js "${user.jobs}[0]"
  And job 2 is defined by js "${user.jobs}[1]"
  And job count is defined by js "${user.jobs}.length"
```

### Looping Nested Arrays

> Since [v3.65.0](https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.65.0)

You can loop over JSON arrays with the following DSL
- [`<step> for each <entry> in <json|JSON> array <arrayRef>`](/docs/dsl#for-each-entry-in-json-array)

Example:

```gherkin
  And I type job in the job field for each job in json array user.jobs
```

### Single JSON Object

> Since [v3.64.1](https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.64.1)
