...
`@Message` annotations can be used to override default error messages reported by failed assertions or timeouts with custom messages to provide better meaning and context.
...
Error messages raised by a series [soft](#soft-assertions) or [sustained](#sustained-assertions) assertions can be aggregated into a single errWith a [cumulative assert](/docs/dsl/assertions/cumulative-asserts).
...
### Usage
  ...
  You can also [reset accumulated errors](/docs/dsl/assertions/cumulative-asserts#i-reset-accumulated-errors) if you need to start a new series of asserts. All currently accumulated errors will be cleared.
