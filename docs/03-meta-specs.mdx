...
Meta specs are [Gherkin](https://cucumber.io/docs/gherkin/reference/) specifications that describe how your features will execute and are the automation glue in Gwen. They define all the step definitions and bindings required for automation and attach to features at runtime.
...
In the extremely rare cases where you need to synchronize your StepDef to a single thread for whatever reason, you can do so by annotating it with `@Synchronized`. Gwen will then guarantee that only one thread can execute it at any given time when multiple features or scenarios are executed in parallel by concurrent threads.

## Meta Load Strategies

### Associative Strategy

When [associative meta](/docs/settings/reference#gwen-associative-meta) is enabled, a meta and feature file are considered to be associated with each other if the two files are in the same directory and have the same names (excluding their file extensions). The associative strategy will, for every feature:
...
Although it's possible to specify imports directly in feature files, we do not recommend you doing so since it would introduce automation concerns into your features. Confining imports to associative meta instead will keep your features clean and untouched. You must enable [associative meta](/docs/settings/reference#gwen-associative-meta) to use this approach (note that this setting will be enabled by default in the upcoming Gwen 3 release).
