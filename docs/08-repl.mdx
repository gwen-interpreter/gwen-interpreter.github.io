...
(args) :=> body
  Evaluates a built in Gwen utility function
...
Develop and experiment with built-in Gwen [utility functions](/docs/utility-functions) by entering them directly in the REPL.

```script
gwen> Given iso date is "2023-10-01"

  [1ms] ✔

gwen> (date = iso date) :=> date.from('yyyy-MM-dd').to('dd/MM/yyyy')
