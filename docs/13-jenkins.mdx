...
Then it's just a matter of calling `docker-compose run gwen` in your Jenkins job or pipeline and everything including a browser environment and Gwen itself (and all it's dependencies) will spin up and run.
...
5. Schedule or run the job in Jenkins
    - Evaluation reports will generated and published to each job execution (Gwen-Report links)
