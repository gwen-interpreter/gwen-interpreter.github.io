...
pipeline {
  parameters { 
    choice(name: 'browser', choices: ['chrome', 'firefox', 'safari', 'edge', 'ie'], description: 'Target web browser')
    ...
    stage("Gwen") {
      steps {
        script {
          ...
          sh "docker-compose run -e GWEN_BROWSER=${params.browser} -e GWEN_DRY_RUN=${params.dry_run} -e GWEN_PARALLEL=${params.parallel} -e GWEN_HEADLESS=${params.headless} gwen"
        }
      }
      post {
        always {
