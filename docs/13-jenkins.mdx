...
To run your Gwen project in Jenkins, get your Gwen project [running in Docker](/docs/docker) first. Then create a Jenkins pipeline file in your project using the [`init --jenkins`](/docs/cli#gwen-cli-options-init-jenkins) command as shown below. This command is available since Gwen v3.16.0. You will need to create the `Jenkinsfile` manually if you are using an older version.
