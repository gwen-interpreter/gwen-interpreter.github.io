...
Sometimes it may be useful to run Gwen in a [docker](https://www.docker.com/) container. For example, if you want to run Gwen in a [Jenkins](/docs/jenkins) pipeline on a build server or node agent.
...
```shell
docker-compose run -e GWEN_PARALLEL=true gwen
```

To run features in parallel and the browser in headless mode

```shell
docker-compose run -e GWEN_PARALLEL=true -e GWEN_WEB_HEADLESS=true gwen
```
