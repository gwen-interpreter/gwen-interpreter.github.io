...
## Dockerfile
The first thing you need to do is create a suitable docker image for Gwen to run in. It will need to have Java 8+, node 12+ and yarn installed (since we will be using yarn in the examples that follow).
...
Next, we need to configure a `docker-compose.yml` file to create docker containers for Gwen and at least one web browser for it to run against. Here, we will use [Selenoid](https://aerokube.com/selenoid/) to stand up a chrome browser and Selenium hub that Gwen will connect to.
