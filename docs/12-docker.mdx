...
Create the following Docker and Selenoid files in your project using the [`init --docker`](/docs/cli#init-command) command as shown below. This command is available since Gwen v3.16.0. You will need to create the files manually if you are using an older version.
...
The init --docker commad will create and populate a `gwen/DockerFile` with the following content to build an Alpine Linux node and Yarn based image with bash and Open JDK 8 JRE installed.
...
The init --docker commad will create and populate a `gwen/docker-compose.yml` file with the following content to create containers for Gwen, Chrome, Firefox, and Edge web browsers, a video recorder, and a Selenoid hub that Gwen will connect to.
...
The init --docker commad will create and populate a `gwen/browsers/browsers.json` file with the following content that will be used by Selenoid to stand up the browsers in Docker.
...
The init --docker commad will create and populate a `gwen/browsers/selenoid.conf` file with the following content that will be used by Gwen to connect to Selenoid.
...
Run the following command in your project directory to spin everything up and launch Gwen in docker
...
```shell
docker-compose -f gwen/docker-compose.yml run -e GWEN_PARALLEL=true gwen
```
...
```
If you set `gwen.web.capability.videoEnabled` to `true` in your `selenoid.conf` file, MP4 video recordings of each session will be attached to the feature level [HTML reports](/docs/reports/html). Set this to `false` if you do not wish to capture video.
...
:::caution Video limitation
Since v3.16.0, no videos will be captured when [parallel execution](/docs/modes/parallel) or [headless browser](/docs/settings/reference#gwen-web-browser-headless) modes are enabled.
:::
