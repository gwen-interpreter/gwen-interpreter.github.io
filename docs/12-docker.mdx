...
Add the following `gwen:selenoid` script to the `scripts` section of the `package.json` file in your project. This will be called by the `gwen` container in the [docker-compose](#docker-compose-file) file generated above to launch Gwen and execute your features in a browser in Selenoid.

```json {3}
"scripts": {
  "gwen": "gwen",
  "gwen:selenoid": "gwen -b -c gwen/browsers/selenoid.conf gwen/features"
}
```
