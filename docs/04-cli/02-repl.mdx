...
To start a REPL session, invoke the [Gwen CLI](/docs/cli/gwen-cli) without specifying the `-b` (or `--batch`) switch.
...
You can press the tab key at any time to lookup and select available commands, [DSL steps](/docs/dsl/gwen-dsl) and custom StepDefs.
...
You will then be able to enter or paste a multiple the step without anything being executed when you press the enter key for each new line. For example, you could enter the following without any lines being executed prematurely:
...
Then when you're ready to execute or want to exit the *paste* mode, insert a new empty line at the end and press press `cntl-D`. The REPL will then evaluate the step you entered before the new line and return to the standard prompt when done.
