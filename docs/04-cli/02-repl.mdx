...
To start a REPL session, invoke the [Launcher CLI](/docs/cli/launcher) without specifying the `-b` (or `--batch`) switch.
...
You can press the tab key at any time to lookup and select available commands, [DSL Steps](/docs/dsl/dsl-steps) and custom StepDefs.
