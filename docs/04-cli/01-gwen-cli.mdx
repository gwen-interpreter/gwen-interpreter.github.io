...
- To generate HTML and JUnit-XML reports also, add `-r target/reports -f html,junit` to the command
...
In addition to the options above, you can also pass any number of Java system properties or JVM options to the Gwen CLI, including:

- `-Dprop.name=value`
  - For passing a system property to the Java process that runs Gwen
  - Will override any same named Gwen [settings](https://github.com/gwen-interpreter/gwen-web/wiki/Runtime-Settings)
  - Will be picked up by `${prop.name}` placedholders in feature and meta specs
- `-J<OPTION>`
  - For passing a JVM option to the Java process that runs Gwen
  - The `-J` prefix will be stripped and `<OPTION>` will be passed through
