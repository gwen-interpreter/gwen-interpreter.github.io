...
Associative meta is enabled by default since  <Link to="https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.0.0">Gwen 3</Link>.
...
```shell
npm run gwen features -m external-meta
```
...
```shell
yarn gwen features -m external-meta
```
...
```shell
pnpm gwen features -m external-meta
```
...
```shell
npm run gwen -m meta/module-1.meta features/spec-1.feature
```
...
```shell
yarn gwen -m meta/module-1.meta features/spec-1.feature
```
...
```shell
pnpm gwen -m meta/module-1.meta features/spec-1.feature
```
...
With [associative meta](/docs/meta/load-strategies#associative-strategy) enabled, you can do away with having to explicitly specify any meta files on CLI calls to Gwen by creating an associative meta file for each feature file and specifying the required meta to import in each.

:::tip Use imports in associative meta

Although it's possible to specify imports directly in feature files, we do not recommend you doing so since it would introduce automation concerns into your features. Confining imports to associative meta instead will keep your features clean and untouched. You must enable [associative meta](/docs/settings/reference#gwen-associative-meta) to use this approach (note that this setting is enabled by default in since  <Link to="https://github.com/gwen-interpreter/gwen-web/releases/tag/v3.0.0">Gwen 3</Link>).
...
```shell
npm run gwen features/spec-1.feature
```
...
```shell
yarn gwen features/spec-1.feature
```
...
```shell
pnpm gwen features/spec-1.feature
```
...
```shell
npm run gwen features/spec-3.feature
```
...
```shell
pnpm gwen features/spec-3.feature
```
