...
2. DSL rule
   - If the step is a DSL step then execute it
3. Undefined rule
   - If neither of the above rules are satisfied then report an undefined step error
   
:::

StepDefs can override DSL steps to alter their operations or perform additional steps around them. If a StepDef calls itself then an error will be reported to prevent infinite recursion.

In the ideal case, all steps will execute. It is your job as a Gwen user to define StepDefs for all non DSL steps in your feature specs (and in your meta if you introduce any there). Gwen will help you by reporting any you may miss as undefined steps. You can also perform [dry runs](https://github.com/gwen-interpreter/gwen/wiki/Execution-Modes#dry-run-execution-and-validation) to check for syntax and binding errors before executing.

## Use DSL steps in meta

:::caution Avoid using DSL steps in features
Although it's possible to use DSL steps directly in feature specs, we do not recommend having DSL-aware features. That's what meta is for!
