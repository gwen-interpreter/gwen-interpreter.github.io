...
If you make changes to a meta file whilst in a REPL console session, you can reload the changes by using the `load` command. This can save you time during meta development so you don't have to end your current REPL session and start a new one to pick up changes. 
...
The current state will not be impacted on reload if your meta declares StepDefs only.

:::

:::caution

Any scenarios in the meta that are not StepDefs will be re-evaluated on reload and could result in side effects that modify the current state. If this becomes a problem, you should refactor or move such scenarios elsewhere so that your meta only declares StepDefs. Otherwise, you might need to manually restore to the prior state by running the necessary steps in the REPL to recover.
