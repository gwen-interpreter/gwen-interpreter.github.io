...
With the above settings in place, evaluation results will be asynchronously sent to your Report Portal server in real time when you launch Gwen to execute your features with the `-f|--format rp` [CLI option](/docs/cli/launcher).
...
To merge current results with any prior results in Report Portal, launch gwen with both the `-Drp.rerun=true` and `-Drp.rerun.of=<launch-uuid>` CLI system property.
...
In addition to the standard [Report Portal configuration](https://github.com/reportportal/client-java#jvm-based-clients-configuration), the folowing can also be configured in your [Gwen settings](/docs/settings/overview) to enable and control more reporting aspects.
...
<details id="gwen-rp-heartbeat">
  ...
  gwen.rp.heartbeat
  ```
  <p>Controls whether or not to enable heartbeats and monitor the Report Portal connection.</p>
  </summary>
  <p>Supported values</p>
  <ul>
