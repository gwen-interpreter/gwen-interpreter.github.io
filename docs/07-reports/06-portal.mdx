...
Configure the following [Report Portal properties](https://github.com/reportportal/client-java#jvm-based-clients-configuration) in your [Gwen settings](/docs/settings/overview):
...
With the above settings in place, evaluation results will be asynchronously logged to your Report Portal server when you launch Gwen to execute your features with the `-f rp` [CLI option](/docs/cli/launcher).
...
### Merging to the previous launch

To merge current results with previous results in Report Portal, launch Gwen with the `-Drp.rerun=true` [CLI system property](/docs/cli/launcher#system-properties-and-jvm-options). This will work as expected only when launching Gwen consecutively on the same machine. You would otherwise need to specifically target the last launch as described next.
...
In addition to the standard [Report Portal configuration](https://github.com/reportportal/client-java#jvm-based-clients-configuration), the following can also be configured in your [Gwen settings](/docs/settings/overview) to enable and control more reporting aspects.
...
<details id="gwen-rp-heartbeat">
  ...
  gwen.rp.heartbeat
  ```
  <p>Controls whether or not to enable heartbeats to monitor the health status of your report portal server during execution.</p>
  </summary>
  <p>Supported values</p>
  <ul>
