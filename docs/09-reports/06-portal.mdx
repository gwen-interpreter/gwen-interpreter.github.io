...
```shell
npm run gwen -- -b -f rp features/todo
```

To generate HTML reports and send results to Report Portal

```shell
npm run gwen -- -b f html,rp features/todo
```
...
```shell
yarn gwen -b -f rp features/todo
```

To generate HTML reports and send results to Report Portal

```shell
yarn  -b f html,rp features/todo
```
...
```shell
pnpm gwen -b -f rp features/todo
```

To generate HTML reports and send results to Report Portal

```shell
pnpm  -b f html,rp features/todo
```
...
<details id="gwen-rp-heartbeat-enabled">
  ...
  <p>Controls whether or not to enable heartbeats to monitor the health status of your report portal server during execution.
    <br/><br/>Formerly `gwen.rp.heartbeat` (prior to v3.0.0)
  </p>
  </summary>
  <p>Supported values</p>
  <ul>
