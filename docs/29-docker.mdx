...
:::caution Limitation
Videos will not be recorded when [parallel execution](/docs/execution-modes#parallel), [headless browser](/docs/settings/reference#gwen-web-browser-headless), or interactive (non [--batch/-b](/docs/cli#gwen-cli-options-batch)) modes are enabled.
:::
