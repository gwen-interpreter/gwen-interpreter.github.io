...
`@DryRun` annotations can be used on any step to assign a value (or values) to a named reference at dry run time and are very useful when: 
...
In this instance, Gwen will have assigned the `$[dryValue:javascript]` placeholder value to the `numbers` binding on line 10 since it is defined with a JavaScript expression which cannot be evaluated in a dry run. This placeholder value would then used to feed the loop, and since it has no spaces in it, the entire value itself becomes the sole undelimited value.

The solution is to assign a dry run value `1 2` to the `numbers` binding as follows:
...
Now when you perform a dry run, the `numbers` reference will take on the value `1 2` and both our step definitions will be called in the the loop on line 11:

```gherkin
  Given I perform step 1
  Given I perform step 2
```
