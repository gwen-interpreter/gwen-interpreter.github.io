...
```gherkin {2} showLineNumbers
  Given number can be located by css ".n"
   When I perform step ${number}    @DryRun(name='number',value={'1','2'})
   Then ..
```

Now when you perform a dry run on this, the `${number}` reference will take on the value `1` followed by `2` and the step on line 2 above will be expanded in-place into the following two steps:
