...
- Or a reference to a binding cannot be evaluated and causes failures that otherwise would not occur outside of a dry run
...
This will result in a successful dry run of both the 1st and 2nd step definitions. So now the entire meta and feature will be exercised.
...
The solution is to assign a dry run value of `1 2` to the `numbers` binding as follows:
...
:::note
You can also source values from settings in annoations using the `${setting.name}` syntax in the `value` attribute, should you wish to externalise your dry run values.
:::