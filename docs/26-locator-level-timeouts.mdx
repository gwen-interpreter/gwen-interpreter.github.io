...
The timeout annotation can be used used on various [wait](/docs/dsl/actions/sleeps-and-waits), [until/while](/docs/dsl/control-structures/until-while), [for each](/docs/dsl/control-structures/foreach), [assertion](/docs/dsl/assertions) or [locator](/docs/dsl/bindings/element-locators) steps.

> #### Delay Annotation
> The same duration patterns also apply to `@Delay` annotations used to control the delay time between repeated operations in the various [wait](/docs/dsl/actions/sleeps-and-waits), [until/while](/docs/dsl/control-structures/until-while) loops. 
