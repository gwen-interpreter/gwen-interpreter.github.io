...
The `Parallel` annotion can be used on examples to force each entry to run in parallel
...
>  - Will come to effect only when Gwen is invoked without the [`--parallel`](/docs/cli#gwen-cli-options-parallel) option (i.e: when no other parallel execution is taking place), and will be implicity removed otherwise.
