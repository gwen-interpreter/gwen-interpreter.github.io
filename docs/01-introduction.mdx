...
For example, the following meta spec describes how the feature spec above can be executed to automate a test against a running [todo application](https://todomvc.com/examples/react).
...
Each step definition is declared as a `@StepDef` annotated scenario with a name that matches that of the step in the feature it binds to. Each one calls out to one or more [DSL steps](/docs/dsl/supported-dsl) (or potentially other StepDefs) in its body to perform the specified operations at run time.
