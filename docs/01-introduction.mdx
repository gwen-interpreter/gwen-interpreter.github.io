...
- Multiple web element [locator mechanisms](/docs/dsl/bindings/element-locators) (including JS)
...
For example, the following meta describes how each step in the scenario above can be executed to automate a process on a running [todo application](https://todomvc.com/examples/react/dist).
...
- [`<element> can be located by <selector> "<expression>"`](/docs/dsl/bindings/element-locators#element-can-be-located-by-selector-expression)
...
- [`I navigate to "<url>"`](/docs/dsl/actions/page-actions#i-navigate-to-url)
...
We could have just hard coded the https://todomvc.com/examples/react/dist URL in the above step, but externalising it enables us to target different environments through [string interpolation](/docs/string-interpolation) without changing our meta. This is useful during development when you want to execute against a locally running instance of the application or for verification when it is running on a different host or port. With this approach you can define a [settings](/docs/settings) file per environment and assign the URL for that environment to the `todo.page.url` property in each file. Then when you want to target an environment, you pass the settings file for that environment to Gwen and the URL will be resolved accordingly.
...
- [`the page title should[ not] <match> "<expression>"`](/docs/dsl/assertions/page-title-asserts#the-page-title-should-match-expression)
...
- [`<element> should[ not] be <state>`](/docs/dsl/assertions/element-asserts#element-should-be-state)
...
- [`I <enter|type> <textRef> in <element>`](/docs/dsl/actions/element-actions#i-enter-or-type-text-ref-in-element)
- [`<element> should[ not] <match> <expressionRef>`](/docs/dsl/assertions/element-asserts#element-should-match-expression-ref)
...
- [`<element> should[ not] <match> "<expression>"`](/docs/dsl/assertions/element-asserts#element-should-match-expression)
...
<p>
You can use Gwen to automate testing or robotic processing!
</p>
:::
