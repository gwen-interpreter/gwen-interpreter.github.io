...
Each step definition is declared as a `@StepDef` annotated scenario with a name that matches that of the step in the feature it binds to. Each one calls out to one or more [DSL steps](/docs/dsl/overview) (or potentially other StepDefs) to perform the specified operations at run time.
