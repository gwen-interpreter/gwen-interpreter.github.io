...
Meta specifications are used to describe how Gwen will execute features and are maintained in Gherkin meta files. All the step definitions and bindings required for automation are defined in meta so your features remain unconcerned with this detail.

For example, the following meta specs describes how the feature spec above can be executed to automate a test against a running [todo application](https://todomvc.com/examples/react).
